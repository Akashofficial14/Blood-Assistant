...
import { useGetAllUsers } from "./hooks/useAdminApi";
import { Eye, EyeOff, UserPlus, MoreVertical, ShieldAlert, CheckCircle2 } from "lucide-react";
...
const UserManagement = ({ searchTerm }) => {
 ...
 console.log("All Users Data from React Query:", allUsers);
  ...
  const handleBulkUpdate = () => {
    // API CALL HERE: 
    // updateBulkRoles({ userIds: selectedUsers, newRole: targetRole })
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-50">
            {filteredUsers.map((user) => (
              <tr key={user._id} className={`transition-colors ${selectedUsers.includes(user._id) ? 'bg-red-50/30' : 'hover:bg-gray-50/50'}`}>
                ...
                <td className="px-8 py-5">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-sm text-gray-400 font-mono">
                      {visibleEmails[user._id] ? user.email : "••••••••@••••.com"}
                    </span>
                    <button onClick={() => toggleEmail(user._id)} className="text-gray-300 hover:text-red-500 transition-colors">
                      {visibleEmails[user._id] ? <EyeOff size={14} /> : <Eye size={14} />}
                    </button>
                  </div>
                </td>
                <td className="px-8 py-5 text-center">
                  <span className="px-4 py-1.5 bg-gray-100 text-gray-600 text-[10px] font-black rounded-full uppercase tracking-widest">
                    {user.userRole === 'manage_bank' ? 'Bank Manager' : user.userRole}
                  </span>
                </td>
                <td className="px-8 py-5 text-right font-bold text-xs text-gray-400">
                  {user.isVerified ? <span className="text-green-500">Verified</span> : "Pending"}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {isBulkModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-3xl p-8 shadow-2xl">
            <h2 className="text-xl font-black text-gray-900 mb-2">Update Roles</h2>
            <p className="text-gray-500 text-sm mb-6">Select a new role for the {selectedUsers.length} selected users.</p>
              ...
              {['admin', 'user'].map((role) => (
                <button
                  ...
                  className={`w-full p-4 rounded-2xl text-left font-bold text-sm transition-all border-2 ${
                    targetRole === role ? 'border-red-500 bg-red-50 text-red-600' : 'border-gray-100 text-gray-600'
                  }`}
                >
            ...
            <div className="flex gap-3 mt-8">
              <button onClick={() => setIsBulkModalOpen(false)} className="flex-1 py-4 font-bold text-gray-400 hover:text-gray-600">Cancel</button>
              <button 
